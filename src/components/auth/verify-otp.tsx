...
import { useVerifyOtpMutation } from "@/src/store/features/otp/otp.features";
...
export default function VerifyOtpPage() {
  ...
  const onSubmit: SubmitHandler<IVerfiyOTP> = async(data: IVerfiyOTP) => {
        ...
        >
          ...
          <View className="flex-row justify-between mb-10">
            {[0, 1, 2, 3].map((index) => (
              <View
                key={index}
                className="w-[70px] h-[70px] border border-[#E1E8ED] rounded-2xl bg-white items-center justify-center"
              >
                <TextInput
          ...
          <TouchableOpacity onPress={()=>onSubmit} className="bg-[#00AA45] h-14 rounded-xl justify-center items-center mb-8 shadow-lg shadow-[#00AA45]/20">
          ...
          {/* Footer Link */}
          <TouchableOpacity className="flex-row items-center justify-center mt-6" onPress={navigationRouter.goLogin}>
